...
export class Marked {
      ...
      if (pack.renderer) {
        ...
        for (const prop in pack.renderer) {
          ...
          renderer[rendererProp] = (...args: unknown[]) => {
            ...
            // Return empty span instead of empty string
            return ret || h('span');
          };
        }
  ...
  private parseMarkdown(blockType: boolean) {
    type overloadedParse = {
      ...
      (src: string, options?: MarkedOptions | undefined | null): VNode[] | Promise<VNode[]>;
    };
    ...
    const parse: overloadedParse = (src: string, options?: MarkedOptions | undefined | null): any => {
      ...
      try {
        ...
        const nodes = parser(tokens, opt);
        if (opt.hooks) {
          return opt.hooks.postprocess(nodes);
        }
        return nodes;
      } catch (e) {
  ...
  private onError(silent: boolean, async: boolean) {
    return (e: Error): VNode[] | Promise<VNode[]> => {
      e.message += '\nPlease report this to https://github.com/markedjs/marked.';

      if (silent) {
        const errorNodes = [
          h('p', {}, 'An error occurred:'),
          h('pre', {}, escape(e.message + '', true))
        ];
        
        if (async) {
          return Promise.resolve(errorNodes);
        }
        return errorNodes;
      }
...
function h(tag: string, props?: Record<string, any>, children?: string | VNode[]): VNode {
